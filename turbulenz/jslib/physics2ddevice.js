...
var Physics2DConstraint = (function () {
    ...
    Physics2DConstraint.prototype._inWorld = function () { debug.abort("abstract method"); };
    Physics2DConstraint.prototype._outWorld = function () { debug.abort("abstract method"); };
    Physics2DConstraint.prototype._pairExists = function (b1, b2) { debug.abort("abstract method"); return false; };
    Physics2DConstraint.prototype._wakeConnected = function () { debug.abort("abstract method"); };
    Physics2DConstraint.prototype._sleepComputation = function (union) { debug.abort("abstract method"); };
    Physics2DConstraint.prototype._preStep = function (deltaTime) { debug.abort("abstract method"); return false; };
    Physics2DConstraint.prototype._warmStart = function () { debug.abort("abstract method"); };
    Physics2DConstraint.prototype._iterateVel = function () { debug.abort("abstract method"); return false; };
    Physics2DConstraint.prototype._iteratePos = function () { debug.abort("abstract method"); return false; };
    ...
    Physics2DConstraint.prototype._clearCache = function () { debug.abort("abstract method"); };
        ...
        for (i = 0; i < dimension; i += 1) {
            ...
            for (j = i; j < dimension; j += 1) {
                ...
                for (k = 0; k < limit; k += 1) {
                    ...
                    var k3 = (k * 3);
                    sum += (body[(0)] * ((data[JACI + k3] * data[JACJ + k3]) +
...
var Physics2DPulleyConstraint = (function (_super) {
    ...
    Physics2DPulleyConstraint.prototype._posError = function () {
        ...
        var jointMin = data[(5)];
        var jointMax = data[(6)];
        var n12x = ((b2[(2)] + data[(21)]) -
            (b1[(2)] + data[(19)]));
        var n12y = ((b2[(2) + 1] + data[(21) + 1]) -
            (b1[(2) + 1] + data[(19) + 1]));
        var n34x = ((b4[(2)] + data[(25)]) -
            (b3[(2)] + data[(23)]));
        var n34y = ((b4[(2) + 1] + data[(25) + 1]) -
            (b3[(2) + 1] + data[(23) + 1]));
    ...
    Physics2DPulleyConstraint.prototype._preStep = function (deltaTime) {
        ...
        var im3 = b3[(0)];
        var ii3 = b3[(1)];
        var K = (b1[(0)] + b2[(0)] + (ratioSq * (im3 + b4[(0)])) +
            (cx1 * b1[(1)] * cx1) + (cx2 * b2[(1)] * cx2) + (cx3 * ii3 * cx3) +
            (cx4 * b4[(1)] * cx4));
        if (b2 === b3) {
...
var Physics2DShape = (function () {
    ...
    Physics2DShape.prototype.addEventListener = function (eventType, callback, callbackMask, deterministic) {
        ...
        if (eventType === 'preSolve') {
            ...
            events = this._events;
            type = (eventType === 'begin' ? (1) :
                eventType === 'progress' ? (2) :
                    eventType === 'end' ? (3) :
                        null);
        }
        if (type === null) {
...
var Physics2DPolygon = (function (_super) {
    ...
    Physics2DPolygon.prototype.computeArea = function () {
        ...
        for (; index < limit; index += (13)) {
            ...
            doubleArea += ((data[index + (0)] * data[next + (0) + 1]) -
                (data[index + (0) + 1] * data[next + (0)]));
        }
    ...
    Physics2DPolygon.prototype._update = function (posX, posY, cos, sin, skipAABB) {
        ...
        j = data.length - (13);
        data[j + (11)] =
            ((data[j + (6)] * data[index + (2) + 1]) -
                (data[j + (6) + 1] * data[index + (2)]));
        if (!skipAABB) {
            data[(0)] = minX;
            data[(0) + 1] = minY;
            data[(0) + 2] = maxX;
...
var Physics2DRigidBody = (function () {
    ...
    Physics2DRigidBody.create = function (params) {
        ...
        var inf = Number.POSITIVE_INFINITY;
        b._type = (params.type === 'dynamic' ? (0) :
            params.type === 'static' ? (2) :
                params.type === 'kinematic' ? (1) :
                    (0));
    ...
    Physics2DWorld.create = function (params) {
        ...
        w._convexCast = {
            ...
            sample: function sampleFn(handle, _) {
                ...
                toi.shapeB = shape;
                shape.body._update();
                var ret = w._collisions._staticSweep(toi, (this.minTOIAlpha * this.deltaTime), 0) *
                    this.minTOIAlpha;
                if (ret <= 0) {
                    return;
                }
