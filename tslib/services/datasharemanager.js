        ...
        var dataShareJoinCallback = function dataShareJoinCallbackFn(jsonResponse, status) {
            ...
            if (status === 200) {
                ...
                errorCallback("DataShare.join failed with " +
                    "status " + status + ": " + jsonResponse.msg, status, that.join, [callbackFn]);
            }
        };
        ...
        var dataShareSetJoinableCallback = function dataShareSetJoinableCallbackFn(jsonResponse, status) {
            ...
            if (status === 200) {
                ...
                errorCallback("DataShare.setJoinable failed with " +
                    "status " + status + ": " + jsonResponse.msg, status, that.setJoinable, [joinable, callbackFn]);
            }
        };
        if (joinable) {
            joinable = 1;
        ...
        var dataShareLeaveCallback = function dataShareLeaveCallbackFn(jsonResponse, status) {
            ...
            if (status === 200 || status === 403 || status === 404) {
                ...
                errorCallback("DataShare.leave failed with " +
                    "status " + status + ": " + jsonResponse.msg, status, that.leave, [callbackFn]);
            }
        };
        this.service.request({
        ...
        var dataShareGetKeysCallback = function dataShareGetKeysCallbackFn(jsonResponse, status) {
            ...
            if (status === 200) {
                ...
                errorCallback("DataShare.getKeys failed with " +
                    "status " + status + ": " + jsonResponse.msg, status, that.getKeys, [callbackFn]);
            }
        };
        this.service.request({
        ...
        var dataShareGetCallback = function dataShareGetCallbackFn(jsonResponse, status) {
            ...
            if (status === 200) {
                ...
                errorCallback("DataShare.get failed with " +
                    "status " + status + ": " + jsonResponse.msg, status, that.get, [key, callbackFn]);
            }
        };
        this.service.request({
                ...
                if (reason) {
                    ...
                    if (errorCallback) {
                        errorCallback("DataShare.set failed with " +
                            "status " + status + ": " + jsonResponse.msg, status, that.set, [params]);
                    }
                }
            }
        };
            ...
            if (status === 200) {
                ...
                if (reason) {
                    ...
                    errorCallback("DataShare.compareAndSet failed with " +
                        "status " + status + ": " + jsonResponse.msg, status, that.compareAndSet, [params]);
                }
            }
        };
        var dataSpec = {
            ...
            if (status === 200) {
                ...
                if (errorCallback) {
                    errorCallback("DataShareManager.createDataShare failed with " +
                        "status " + status + ": " + jsonResponse.msg, status, that.createDataShare, [callbackFn, errorCallbackFn]);
                }
            }
        };
        var dataSpec = {
        ...
        var findDataSharesCallback = function findDataSharesCallbackFn(jsonResponse, status) {
            ...
            if (status === 200) {
                ...
                errorCallback("DataShareManager.findDataShares failed with " +
                    "status " + status + ": " + jsonResponse.msg, status, that.findDataShares, [params]);
            }
        };
        var dataSpec = {};
        if (params.user) {
