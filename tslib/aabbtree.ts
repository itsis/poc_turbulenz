...
class AABBTreeNode
{
    ...
    reset(minX: number, minY: number, minZ: number,
          maxX: number, maxY: number, maxZ: number,
          escapeNodeOffset: number, externalNode?: {}): void
    {
        this.escapeNodeOffset = escapeNodeOffset;
        this.externalNode = externalNode;
...
class AABBTree
{
    ...
    findParent(nodeIndex: number): AABBTreeNode
    {
        var nodes = this.nodes;
    ...
    _sortNodes(nodes: AABBTreeNode[]): void
    {
        var numNodesLeaf = this.numNodesLeaf;
        var numNodes = nodes.length;

        function getkeyXfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return (extents[0] + extents[3]);
        }

        function getkeyYfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return (extents[1] + extents[4]);
        }

        function getkeyZfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return (extents[2] + extents[5]);
        }

        function getreversekeyXfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return -(extents[0] + extents[3]);
        }

        function getreversekeyYfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return -(extents[1] + extents[4]);
        }

        function getreversekeyZfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return -(extents[2] + extents[5]);
        }
    ...
    _sortNodesHighQuality(nodes: AABBTreeNode[]): void
    {
        ...
        function getkeyXZfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return (extents[0] + extents[2] + extents[3] + extents[5]);
        }

        function getkeyZXfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return (extents[0] - extents[2] + extents[3] - extents[5]);
        }
        ...
        function getreversekeyXZfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return -(extents[0] + extents[2] + extents[3] + extents[5]);
        }

        function getreversekeyZXfn(node: AABBTreeNode): number
        {
            var extents = node.extents;
            return -(extents[0] - extents[2] + extents[3] - extents[5]);
        }
    ...
    _calculateSAH(buildNodes: AABBTreeNode[], startIndex: number, endIndex: number): number
    {
    ...
    _nthElement(nodes: AABBTreeNode[], first: number, nth: number, last: number, getkey: (AABBTreeNode) => number): void
    {
        function medianFn(a: number, b: number, c: number): number
        {
            if (a < b)
            {
                if (b < c)
        ...
        function insertionSortFn(nodes: AABBTreeNode[],
                                 first: number,
                                 last: number,
                                 getkey: (AABBTreeNode) => number): void
        {
            var sorted = (first + 1);
            while (sorted !== last)
            {
    ...
    _recursiveBuild(buildNodes: AABBTreeNode[], startIndex: number, endIndex: number, lastNodeIndex: number): void
    {
        var nodes = this.nodes;
    ...
    _predictNumNodes(startIndex: number, endIndex: number, lastNodeIndex: number): number
    {
        lastNodeIndex += 1;
    ...
    getVisibleNodes(planes: any[], visibleNodes: {}[], startIndex?: number): number
    {
    ...
    getOverlappingNodes(queryExtents: any, overlappingNodes: {}[], startIndex?: number): number
    {
        if (this.numExternalNodes > 0)
        {
    ...
    getSphereOverlappingNodes(center: any, radius: number, overlappingNodes: {}[]): void
    {
        if (this.numExternalNodes > 0)
        {
    ...
    getOverlappingPairs(overlappingPairs: {}[], startIndex: number): number
    {
        if (this.numExternalNodes > 0)
        {
            var nodes = this.nodes;
    ...
    static rayTest(trees: AABBTree[],
                   ray: AABBTreeRay,
                   callback: (tree: AABBTree,
                              externalNode: {},
                              ray: AABBTreeRay,
                              distance: number,
                              upperBound: number) => AABBTreeRayTestResult): AABBTreeRayTestResult
    {
        ...
        function distanceExtents(extents: any, upperBound: number): number
        {
            var min0 = extents[0];
            var min1 = extents[1];
            var min2 = extents[2];
        ...
        function processNode(tree: AABBTree, nodeIndex: number, upperBound: number): number
        {
